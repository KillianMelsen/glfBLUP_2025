# This script takes the datafiles generated by the script at `timing/generate_timing_data.R`
# and then randomly chooses a training and test set, followed by scaling the data.
#
# This script runs in a few seconds, so the easiest way to check reproducibility
# is to simply run the entire script and compare the produced datafiles (training and
# test sets) against the ones already saved to disk. Also note that these files
# are only used for timing. The timing results are not perfectly reproducible anyway
# and obviously depend on hardware.

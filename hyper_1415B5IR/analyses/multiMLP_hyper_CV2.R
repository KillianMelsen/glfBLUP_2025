# This script produces all intermediate results for the multiMLP CV2 analyses on
...
#
# Suppose you want to reproduce the result of replication 2. First, manually
# set `lab <- "b"`. Then set `last <- 2` instead of setting it to 250 on line 138.
# Running the loop will then produce the CV2 accuracy of replication 2 as the 2nd value in `acc`.
# This can be compared to the 2nd accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/8%s_hyper_results_multiMLP_%s.csv", prep, lab, CV))$acc[2],
#           acc[2])
#
# [1] TRUE

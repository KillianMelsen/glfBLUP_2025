# This script produces all intermediate results for the multiMLP CV1 analyses on
# the hyperspectral B5IR dataset.
#
# The script is structured as follows:
# - There is a single loop that produces all 250 replications.
# - The seed is set before starting this loop on line 31.
#
# Suppose you want to reproduce the result of replication 5. First, manually
# set `lab <- "a"`. Then set `last <- 5` instead of setting it to 250 on line 134.
# Running the loop will then produce the CV1 accuracy of replication 5 as the 5th value in `acc`.
# This can be compared to the 5th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/8%s_hyper_results_multiMLP_%s.csv", prep, lab, CV))$acc[5],
#           acc[5])
#
# [1] TRUE

# This script produces all intermediate results for the multiMLP CV2VEG analyses on
# the hyperspectral B5IR dataset.
#
# The script is structured as follows:
# - There is a single loop that produces all 250 replications.
# - The seed is set before starting this loop on line 35.
#
# Suppose you want to reproduce the result of replication 8. First, manually
# set `lab <- "b"`. Then set `last <- 8` instead of setting it to 250 on line 138.
# Running the loop will then produce the CV2VEG accuracy of replication 8 as the 8th value in `acc`.
# This can be compared to the 8th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/8%s_hyper_results_multiMLP_%sVEG.csv", prep, lab, CV))$acc[8],
#           acc[8])
#
# [1] TRUE
...
# Load marker data:
load("genotypes/M_hyper.RData"); load("genotypes/K_hyper.RData")
...
for (run in first:last) {
  ...
  if (mean(mean.accs) != 0) {
    ...
    cat(sprintf("Applying RC2019 correction...\n"))
...
# Export results:
write.csv(results, sprintf("hyper_1415B5IR/results/%s/8%s_hyper_results_multiMLP_%sVEG.csv",
                           prep, lab, CV))

list.save(histories, sprintf("hyper_1415B5IR/results/%s/8%s_hyper_results_multiMLP_%sVEG.RData",
                             prep, lab, CV))

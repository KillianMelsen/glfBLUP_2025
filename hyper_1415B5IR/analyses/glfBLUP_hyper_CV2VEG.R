...
#
# Suppose you want to reproduce the result of replication 54. Looking at the `work`
# variable, replication 54 is handled by the 2nd worker. So we can manually set
# `i <- 2` instead of running the parallel for loop. We also note that replication
# 54 is the 4th replication that is produced by the 2nd worker, so in the inner
# loop we do not go from `1:length(par.work)`, but `1:4`. Then running the loop
# will produce the CV2VEG accuracy of replication 54 as the 4th value in `CV2.acc`.
# This can be compared to the 54th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/3b_hyper_results_glfblup_CV2VEG.csv", prep))$acc[54],
#           CV2.acc[4])
#
# [1] TRUE
...
invisible(
  par.results <- foreach::foreach(i = 1:length(work), .packages = c("rlist", "tictoc", "gfBLUPold", "glfBLUP"), .combine = "c") %dopar% {
    ...
    # Collect results:
    worker.result <- list(list(result = data.frame(CV2.acc = CV2.acc,
                                                   penG = penG,
                                                   penE = penE,
                                                   subset = subset,
                                                   comptimes = comptimes),
...
# Export results:
write.csv(CV2.results, sprintf("hyper_1415B5IR/results/%s/3b_hyper_results_glfblup_CV2VEG.csv", prep))

list.save(extra, sprintf("hyper_1415B5IR/results/%s/3_hyper_extra_results_glfblup_CV2VEG.RData", prep))

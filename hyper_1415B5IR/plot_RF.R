# This script produces figure S9 of the manuscript supplementary material.
# Make sure that the intermediate result file loaded on line 7 is available.
# Also make sure the datafile loaded on line 13 is available.

set.seed(1997)
results <-
  rlist::list.load("hyper_1415B5IR/results/splines/11a_hyper_extra_results_lsblup_CV1.RData")

i <- sample(1:250, 1)
results <- results[[i]]
results <- names(results$coefs)

temp <- colnames(rlist::list.load("hyper_1415B5IR/datasets/splines/hyper_dataset_1.RData")$data)
...
library(ggplot2)
library(grid)
data$Date <- factor(data$Date, levels = levels(data$Date), labels = c("2015-01-10", "2015-01-19",
                                                                      "2015-02-04", "2015-02-09",
                                                                      "2015-02-25", "2015-03-10",
...
ggsave(filename = "plots/SM_figure_S9.png", dpi = 640, width = 15, height = 10, units = "cm")

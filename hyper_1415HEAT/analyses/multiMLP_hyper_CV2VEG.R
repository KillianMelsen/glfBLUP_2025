...
# - The seed is set before starting this loop on line 34.
#
# Suppose you want to reproduce the result of replication 21. First, manually
# set `lab <- "b"`. Then set `last <- 21` instead of setting it to 250 on line 137.
# Running the loop will then produce the CV2VEG accuracy of replication 21 as the 21st value in `acc`.
# This can be compared to the 21st accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/8%s_hyper_results_multiMLP_%sVEG.csv", prep, lab, CV))$acc[21],
#           acc[21])
#
# [1] TRUE

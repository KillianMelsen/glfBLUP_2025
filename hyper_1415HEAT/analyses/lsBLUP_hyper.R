...
#
# Suppose you want to reproduce the result of replication 6 (CV2). First, manually
# set `CV <- "CV2"` and `lab <- "b"`. Looking at the `work` variable, replication 6 is handled by
# the 1st worker. So we can manually set `i <- 1` instead of running the parallel
# for loop. We also note that replication 6 is the 6th replication that is
# produced by the 1st worker, so in the inner loop we do not go from
# `1:length(par.work)`, but `1:6`. Then running the loop
# will produce the CV2 accuracy of replication 6 as the 6th value in `acc`.
# This can be compared to the 6th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/11%s_hyper_results_lsblup_%s.csv", prep, lab, CV))$acc[6],
#           acc[6])
#
# [1] TRUE
...
for (CV in c("CV1", "CV2")) {
  ...
  # Export results:
  write.csv(results, sprintf("hyper_1415HEAT/results/%s/11%s_hyper_results_lsblup_%s.csv", prep, lab, CV))
  
  list.save(extra, sprintf("hyper_1415HEAT/results/%s/11%s_hyper_extra_results_lsblup_%s.RData", prep, lab, CV))

...
#
# Suppose you want to reproduce the result of replication 4. Looking at the `work`
# variable, replication 4 is handled by the 1st worker. So we can manually set
# `i <- 1` instead of running the parallel for loop. We also note that replication
# 4 is the 4th replication that is produced by the 1st worker, so in the inner
# loop we do not go from `1:length(par.work)`, but `1:4`. Then running the loop
# will produce the CV1 and CV2 accuracies of replication 4 as the 4th value in `CV2.acc`.
# This can be compared to the 4th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/3a_hyper_results_glfblup_CV1.csv", prep))$acc[4],
#           CV1.acc[4])
#
# [1] TRUE
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/3b_hyper_results_glfblup_CV2.csv", prep))$acc[4],
#           CV2.acc[4])
#
# [1] TRUE
...
# Export results:
write.csv(CV1.results, sprintf("hyper_1415HEAT/results/%s/3a_hyper_results_glfblup_CV1.csv", prep))
write.csv(CV2.results, sprintf("hyper_1415HEAT/results/%s/3b_hyper_results_glfblup_CV2.csv", prep))

list.save(extra, sprintf("hyper_1415HEAT/results/%s/3_hyper_extra_results_glfblup.RData", prep))

...
#
# Suppose you want to reproduce the result of replication 13. First, manually
# set `lab <- "b"`. Then set `last <- 13` instead of setting it to 250 on line 138.
# Running the loop will then produce the CV2 accuracy of replication 13 as the 13th value in `acc`.
# This can be compared to the 13th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/8%s_hyper_results_multiMLP_%s.csv", prep, lab, CV))$acc[13],
#           acc[13])
#
# [1] TRUE

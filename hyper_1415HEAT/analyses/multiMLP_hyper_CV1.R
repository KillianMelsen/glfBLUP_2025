...
# set `lab <- "a"`. Then set `last <- 2` instead of setting it to 250 on line 134.
# Running the loop will then produce the CV1 accuracy of replication 2 as the 2nd value in `acc`.
# This can be compared to the 2nd accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/8%s_hyper_results_multiMLP_%s.csv", prep, lab, CV))$acc[2],
#           acc[2])
#
# [1] TRUE

# This script produces all intermediate results for the MegaLMM CV2 analyses on
# the hyperspectral HEAT dataset.
#
# The script is structured as follows:
# - There is a single loop that produces all 250 replications.
#
# Note that MegaLMM's results are not perfectly reproducible given the random nature
# of the MCMC procedures used. Please refer to the 'MegaLMM reproducibility' section
# of the readme for further information.
...
for (run in datasets) {
  ...
  # Creating run ID:
  run_ID <- sprintf("hyper_1415HEAT/megalmm_states/%s_%s_hyper_dataset_%d_RF", prep, CV, run)
...
# Export results:
write.csv(results, sprintf("hyper_1415HEAT/results/%s/12%s_hyper_results_megalmm_%s.csv", prep, lab, CV))

...
# replications for the hyperspectral HEAT data.
#
...
#
# Suppose we want to check the datafile for replication 24. We first manually set `n.datasets <- 24`
...
# simply run the loop.
# We can then check the data stored in the variables `datalist.nosplines` and `datalist.splines`
# against the datafiles already stored on disk:
#
# all.equal(rlist::list.load(sprintf("hyper_1415HEAT/datasets/nosplines/hyper_dataset_%d.RData", run)),
#           datalist.nosplines)
# [1] TRUE
#
# all.equal(rlist::list.load(sprintf("hyper_1415HEAT/datasets/splines/hyper_dataset_%d.RData", run)),
#           datalist.splines)
# [1] TRUE
...
for (run in 1:n.datasets) {
  ...
  colnames(pseudoCRD.nosplines)[4:(ncol(pseudoCRD.nosplines) - 2)] <-
    substr(colnames(pseudoCRD.nosplines)[4:(ncol(pseudoCRD.nosplines) - 2)], 2, 13)

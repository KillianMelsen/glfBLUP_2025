...
#
# Suppose you want to reproduce the result of replication 6 for `CV = "CV1"`, `lab = "a"`,
# `h2s = "05"`, `h2y = "09"`, and `comm = "02"`. The first step is to set these values
# manually instead of relying on the two outer loops (CV and h2s), and assignments on
# lines 73 (comm) and 74 (h2y). Then we set `n.sim <- 6` instead of setting it to
# 100 on line 77. Then running the inner loop will produce the correct accuracy for
# replication 6 as the 6th value in `acc`.
# This can be compared to the 6th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("p800/results/h2s%s/1%s_p800_results_benchmark_%s_h2y%s_comm%s_h2s%s.csv", h2s, lab, CV, h2y, comm, h2s))$acc[6],
#           acc[6])
#
# [1] TRUE
...
for (CV in CVs) {
  for (h2s in h2.sec) {
    ...
    cl <- parallel::makeCluster(15, outfile = sprintf("logs/benchmark_sim_p800_h2s%s_%s.txt", h2s, CV))
    ...
    invisible(
      foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
        ...
        for (sim in 1:n.sim) {
          ...
          cat(sprintf("Running benchmark on %s p800 dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
        ...
        # Export results:
        write.csv(results, sprintf("p800/results/h2s%s/1%s_p800_results_benchmark_%s_h2y%s_comm%s_h2s%s.csv",

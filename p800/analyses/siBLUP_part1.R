...
#
# Suppose you want to reproduce the result of replication 4 for `CV = "CV1"`, `lab = "a"`,
# `h2s = "05"`, `h2y = "01"`, and `comm = "05"`. The first step is to set these values
# manually instead of relying on the two outer loops (CV and h2y), and assignments on
# lines 71 (comm) and 72 (h2s). Then we set `last <- 4` instead of setting it to
# 50 on line 76. Then running the inner loop will produce the correct accuracy for
# replication 4 as the 4th value in `acc`.
# This can be compared to the 4th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("p800/results/h2s%s/5%s_p800_results_siblup_%s_h2y%s_comm%s_h2s%s.csv", h2s, lab, CV, h2y, comm, h2s))$acc[4],
#           acc[4])
#
# [1] TRUE

...
#
# Suppose you want to reproduce the result of replication 12 for `CV = "CV2"`, `lab = "b"`,
# `h2s = "07"`, `h2y = "03"`, and `comm = "02"`. The first step is to set these values
# manually instead of relying on the two outer loops (CV and h2s), and assignments on
# lines 71 (comm) and 72 (h2y). Then we set `n.sim <- 12` instead of setting it to
# 100 on line 75. Then running the inner loop will produce the correct accuracy for
# replication 12 as the 12th value in `acc`.
# This can be compared to the 12th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("p800/results/h2s%s/11%s_p800_results_lsblup_%s_h2y%s_comm%s_h2s%s.csv", h2s, lab, CV, h2y, comm, h2s))$acc[12],
#           acc[12])
#
# [1] TRUE

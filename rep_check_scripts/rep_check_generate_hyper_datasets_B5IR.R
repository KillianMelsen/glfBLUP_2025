# This script is a copy of the script at `hyper_1415B5IR/data_generation/generate_hyper_datasets.R`.
...
# instead of setting it to 250 on line 38. We then temporarily comment out the code
# that saves the datafiles to disk on lines 100 and 111 to avoid overwriting. We can then
# simply run the loop.
...
cat(paste0(all.equal(rlist::list.load(sprintf("hyper_1415B5IR/datasets/splines/hyper_dataset_%d.RData", run)),
                     datalist.splines), "\n"))

cat(paste0(all.equal(rlist::list.load(sprintf("hyper_1415B5IR/datasets/VEGsplines/hyper_dataset_%d.RData", run)),
                     datalist.VEGsplines), "\n"))
# toc()

# This script is a copy of the script at `hyper_1415B5IR/analyses/glfBLUP_hyper.R`.
...
# This script produces all intermediate results for the glfBLUP CV2 and CV1 analyses on
...
#   inside the inner loop starting on line 76.
#
# Suppose you want to reproduce the result of replication 7. Looking at the `work`
# variable, replication 7 is handled by the 1st worker. So we can manually set
# `i <- 1` instead of running the parallel for loop. We also note that replication
# 7 is the 7th replication that is produced by the 1st worker, so in the inner
# loop we do not go from `1:length(par.work)`, but `1:7`. Then running the loop
# will produce the CV1 and CV2 accuracies of replication 7 as the 7th value in `CV2.acc`.
# This can be compared to the 7th accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/3a_hyper_results_glfblup_CV1.csv", prep))$acc[7],
#           CV1.acc[7])
#
# [1] TRUE
#
# all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/3b_hyper_results_glfblup_CV2.csv", prep))$acc[7],
#           CV2.acc[7])
#
# [1] TRUE
...
# cl <- parallel::makeCluster(n.cores, outfile = "logs/glfBLUP_hyper.txt")
...
cat(paste0(all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/3a_hyper_results_glfblup_CV1.csv", prep))$acc[7],
                     CV1.acc[7]), "\n"))

cat(paste0(all.equal(read.csv(sprintf("hyper_1415B5IR/results/%s/3b_hyper_results_glfblup_CV2.csv", prep))$acc[7],
                     CV2.acc[7]), "\n"))

# We don't need any of the code below for the reproducibility spot check.
...
# 
# # Export results:
# write.csv(CV1.results, sprintf("hyper_1415B5IR/results/%s/3a_hyper_results_glfblup_CV1.csv", prep))
# write.csv(CV2.results, sprintf("hyper_1415B5IR/results/%s/3b_hyper_results_glfblup_CV2.csv", prep))
# 
# list.save(extra, sprintf("hyper_1415B5IR/results/%s/3_hyper_extra_results_glfblup.RData", prep))

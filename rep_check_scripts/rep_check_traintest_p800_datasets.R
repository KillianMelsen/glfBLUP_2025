# This script is a copy of the script at `p800/data_generation/traintest_sim_p800_datasets.R`.
...
# `p800/data_generation/generate_sim_p800_datasets.R`. It selects the training
# and test sets and scales the data separately for training and test set.
#
...
# Suppose we want to check replicate 49 for `h2s = "09"`, `comm = "08"`, and
# `h2y = "07"`. The first step is to set these values manually instead of relying
# on the different loops. Then we set `n.sim <- 49` instead of setting it to 100
# on line 82. Then, we comment out the code on lines 132/133 to prevent overwriting
# the datafile already saved to disk. We also comment out lines 90/91 that would load
# in the datafile already on disk. Finally we comment out all code from line 97 to 129.
# 
...
# replicate 49 to the training and test set according to the file on disk:
#
# all.equal(train.set,
#           rlist::list.load(sprintf("p800/datasets/p800_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$train.set)
#
# [1] TRUE
#
# all.equal(test.set,
#           rlist::list.load(sprintf("p800/datasets/p800_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$test.set)
#
# [1] TRUE
...
cat(paste0(all.equal(train.set,
                     rlist::list.load(sprintf("p800/datasets/p800_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$train.set), "\n"))

cat(paste0(all.equal(test.set,
                     rlist::list.load(sprintf("p800/datasets/p800_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$test.set), "\n"))
      
# We don't need this code for the reproducibility spot check.
#     }
#     toc()

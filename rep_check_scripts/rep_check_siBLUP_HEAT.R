# This script is a copy of the script at `hyper_1415HEAT/analyses/siBLUP_hyper.R`.
...
#
# Suppose you want to reproduce the result of replication 21 (CV1). First, manually
# set `CV <- "CV1"` and `lab <- "a"`. Looking at the `work` variable, replication 21 is handled by
...
# results csv file:
#
# all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/5%s_hyper_results_siblup_%s.csv", prep, lab, CV))$acc[21],
#           acc[7])
#
# [1] TRUE
...
cat(paste0(all.equal(read.csv(sprintf("hyper_1415HEAT/results/%s/5%s_hyper_results_siblup_%s.csv", prep, lab, CV))$acc[21],
                     acc[7]), "\n"))
...
#   
#   # Export results:
#   write.csv(results, sprintf("hyper_1415HEAT/results/%s/5%s_hyper_results_siblup_%s.csv", prep, lab, CV))
#   
#   list.save(extra, sprintf("hyper_1415HEAT/results/%s/5%s_hyper_extra_results_siblup_%s.RData", prep, lab, CV))
#   
# }

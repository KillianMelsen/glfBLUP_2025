# This script is a copy of the script at `p800/data_generation/generate_sim_p800_datasets.R`.
...
#   line 99 produces the 100 datasets for the replications for each combination
...
# on the two outer loops (h2s and comm) and the h2y assignment on line 74. Then we
# set `n.sim <- 13` instead of setting it to 100 on line 85. We then have to comment
# out the code on line 117/118 to avoid overwriting the dataset already saved to disk.
...
cat(paste0(all.equal(rlist::list.load(sprintf("p800/datasets/p800_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$G,
                     datalist$G), "\n"))

#!/usr/bin/env Rscript
# This script produces part 2 of a second, independent set of intermediate results
# (replication 5 to 8) for the MegaLMM p800_lowrank (simulated data with low-rank
  ...
  for (h2y in h2.foc) {
    for (comm in comms) {
      for (h2s in h2.sec) {
        ...
        # Export results:
        write.csv(results, sprintf("MegaLMM_rep_check_scripts/p800_lowrank/results/h2s%s/12%s_p800_lowrank_results_MegaLMM_%s_h2y%s_comm%s_h2s%s_5to8.csv",

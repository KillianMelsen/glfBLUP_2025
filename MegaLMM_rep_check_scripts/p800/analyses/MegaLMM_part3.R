...
# (replication 9 to 12) for the MegaLMM p800 (simulated data with random residual
# structure) analyses.
#
  ...
  for (h2y in h2.foc) {
    for (comm in comms) {
      for (h2s in h2.sec) {
        ...
        # Export results:
        write.csv(results, sprintf("MegaLMM_rep_check_scripts/p800/results/h2s%s/12%s_p800_results_MegaLMM_%s_h2y%s_comm%s_h2s%s_9to12.csv",

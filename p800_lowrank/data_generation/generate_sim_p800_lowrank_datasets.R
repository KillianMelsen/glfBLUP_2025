# This script produces all of the simulated datasets with low-rank residual
# structure (p800_lowrank).
#
...
#
# Suppose we want to reproduce dataset 11 for `h2s = "07"`, `comm = "05"`, and
# `h2y = "01"`. The first step is to set these values manually instead of relying
# on the two outer loops (h2s and comm) and the h2y assignment on line 61. Then we
# set `n.sim <- 11` instead of setting it to 100 on line 72. We then have to comment
...
#
# all.equal(rlist::list.load(sprintf("p800_lowrank/datasets/p800_lowrank_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))$G,
#           datalist$G)
#
# [1] TRUE
...
for (h2s in h2.sec) {
  for (comm in comms) {
    ...
    cl <- parallel::makeCluster(5, outfile = sprintf("logs/generate_p800_lowrank_h2s%s_comm%s.txt", h2s, comm))
    ...
    foreach::foreach(i = 1:length(h2.foc)) %dopar% {
      ...
      for (sim in 1:n.sim) {
        ...
        datalist <- glfBLUP::GFAsim_V2(K = K, r = r, n.LSF = 4, n.LNF = 4,

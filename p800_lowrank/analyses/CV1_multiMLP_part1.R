# This script produces the first 10 of 20 intermediate results for the multiMLP
# CV1 analyses on the simulated datasets with low-rank residual structure (p800_lowrank).
#
# The script is structured as follows:
# - There are three outer loops: the first one is for the focal trait heritabilities
#   ("01", "03", "05", "07", "09"), the second one is for the secondary feature
#   heritabilities ("05", "07", "09"), and the third one is for the communalities
#   ("02", "05", "08")..
# - A single seed is set at the very start of the script, before the three outer loops
#   and the inner loop producing the replications for each combination of focal trait
#   heritability, secondary feature heritability, and communality.
#
# This means that the easiest way to reproduce the intermediate results is to simply run
# the entire script, and compare the produced csv files to the ones already stored in the
# repository.
...
tensorflow::set_random_seed(1997, disable_gpu = FALSE)
...
# Load marker data:
load("genotypes/K_sim.RData"); rm(K)
M <- as.data.frame(M)
M <- cbind(data.frame(G = rownames(M)), M)

# Defining values for the genetic parameters:
h2.sec <- c("05", "07", "09")
comms <- c("02", "05", "08")
...
# Iterating through genetic parameter combinations:
h2y <- "01"; h2s <- "05"; comm <- "02"
for (h2y in h2.foc) {
  for (h2s in h2.sec) {
    for (comm in comms) {
      ...
      run <- 1
      tic(sprintf("%s multiMLP, h2y = %s, h2s = %s, comm = %s", CV, h2y, h2s, comm))
      cat(sprintf("%s multiMLP, h2y = %s, h2s = %s, comm = %s\n\n", CV, h2y, h2s, comm))
      sim <- first
      for (sim in first:last) {
        
        cat(sprintf("### Running on dataset %d / %d (dataset %d, combi %d) ###\n\n", run, n.datasets, sim, combi))
        
        # Loading simulated datasets:
        cat(sprintf("Loading dataset %d...\n", sim))
        datalist <- list.load(sprintf("p800_lowrank/datasets/p800_lowrank_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))
        ...
        if (mean(mean.accs) != 0) {
          ...
          M.test <- M.test[match(names(pred.target), M.test$G),]
      ...
      # Export results:
      write.csv(results, sprintf("p800_lowrank/results/h2s%s/8%s_p800_lowrank_results_multiMLP_%s_h2y%s_comm%s_h2s%s_1to10.csv",
                                 h2s, lab, CV, h2y, comm, h2s))

      list.save(histories, sprintf("p800_lowrank/results/h2s%s/8%s_p800_lowrank_results_multiMLP_%s_h2y%s_comm%s_h2s%s_1to10.RData",
                                   h2s, lab, CV, h2y, comm, h2s))

...
#
# Suppose you want to reproduce the result of replication 6 for `h2s = "05"`,
# `h2y = "01"`, and `comm = "08"`. The first step is to set these values
# manually instead of relying on the outer loop (h2y), and assignments on
# lines 73 (comm) and 74 (h2s). Then we set `last <- 6` instead of setting it to
# 100 on line 78. Then running the inner loop will produce the correct CV1 and CV2
# accuracies for replication 6 as the 6th value in `CV1.acc` and `CV2.acc`.
# These can be compared to the 6th accuracy stored in the appropriate intermediate
# results csv files:
#
# all.equal(read.csv(sprintf("p800_lowrank/results/h2s%s/3a_p800_lowrank_results_glfblup_CV1_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))$acc[6],
#           CV1.acc[6])
#
# [1] TRUE
#
# all.equal(read.csv(sprintf("p800_lowrank/results/h2s%s/3b_p800_lowrank_results_glfblup_CV2_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))$acc[6],
#           CV2.acc[6])
#
# [1] TRUE
...
for (h2y in h2.foc) {
  ...
  tic(sprintf("glfBLUP CV1/CV2 p800_lowrank combi %d / %d, (h2y = %s)", combi, combis, h2y))
  
  cl <- parallel::makeCluster(9, outfile = sprintf("logs/glfBLUP_sim_p800_lowrank_h2y%s.txt", h2y))
  ...
  foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
    ...
    for (sim in first:last) {
      ...
      cat(sprintf("Running glfBLUP CV1/CV2 on p800_lowrank dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
    ...
    # Export results:
    cat(sprintf("Writing results for glfBLUP CV1/CV2 on p800_lowrank dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
                sim, n.sim, h2s, comm, h2y, combi, combis))
    write.csv(CV1.results, sprintf("p800_lowrank/results/h2s%s/3a_p800_lowrank_results_glfblup_CV1_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))
    write.csv(CV2.results, sprintf("p800_lowrank/results/h2s%s/3b_p800_lowrank_results_glfblup_CV2_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))
    
    
    list.save(extra, file = sprintf("p800_lowrank/results/h2s%s/3a_p800_lowrank_extra_results_glfblup_CV1_h2y%s_comm%s_h2s%s.RData", h2s, h2y, comm, h2s))
    list.save(extra, file = sprintf("p800_lowrank/results/h2s%s/3b_p800_lowrank_extra_results_glfblup_CV2_h2y%s_comm%s_h2s%s.RData", h2s, h2y, comm, h2s))

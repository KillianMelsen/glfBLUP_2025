...
#
# Suppose you want to reproduce the result of replication 91 for `h2s = "09"`,
# `h2y = "09"`, and `comm = "08"`. The first step is to set these values
# manually instead of relying on the outer loop (h2s), and assignments on
# lines 67 (comm) and 68 (h2y). Then we set `n.sim <- 91` instead of setting it to
# 100 on line 71. Then running the inner loop will produce the correct accuracy for
# replication 91 as the 91st value in `acc`.
# This can be compared to the 91st accuracy stored in the appropriate intermediate
# results csv file:
#
# all.equal(read.csv(sprintf("p800_lowrank/results/h2s%s/2_p800_lowrank_results_univariate_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))$acc[91],
#           acc[91])
#
# [1] TRUE
...
for (h2s in h2.sec) {
  ...
  cl <- parallel::makeCluster(15, outfile = sprintf("logs/univariate_sim_p800_lowrank_h2s%s.txt", h2s))
  ...
  foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
    ...
    for (sim in 1:n.sim) {
      ...
      cat(sprintf("Running univariate on p800_lowrank dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
    ...
    # Export results:
    write.csv(results, sprintf("p800_lowrank/results/h2s%s/2_p800_lowrank_results_univariate_h2y%s_comm%s_h2s%s.csv",
                               h2s, h2y, comm, h2s))

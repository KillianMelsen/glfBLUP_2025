#!/usr/bin/env Rscript
# This script produces intermediate results 17 to 20 for the MegaLMM CV1 and CV2
...
# - There is then an inner loop starting on line 64 that is responsible for producing
#   intermediate results 17 to 20, for each combination of CV, h2y, h2s, and comm.
#
...
for (CV in CVs) {
  for (h2y in h2.foc) {
    for (comm in comms) {
      for (h2s in h2.sec) {
        tic(sprintf("MegaLMM p800_lowrank combi %d / %d, (CV = %s, h2y = %s, comm = %s, h2s = %s)", # tic 2
        ...
        for (sim in first:last) {
          ...
          cat(sprintf("Running MegaLMM on %s p800_lowrank dataset %d / %d (part %s, dataset %s), (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
          ...
          # Creating run ID:
          run_ID <- sprintf("p800_lowrank/megalmm_states/%s_p800_lowrank_dataset_h2y%s_comm%s_h2s%s_%d", CV, h2y, comm, h2s, sim)
        ...
        # Export results:
        write.csv(results, sprintf("p800_lowrank/results/h2s%s/12%s_p800_lowrank_results_MegaLMM_%s_h2y%s_comm%s_h2s%s_17to20.csv",

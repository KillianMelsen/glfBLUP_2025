# This script produces the 11th to 20th intermediate results for the multiMLP
# CV2 analyses on the simulated datasets with low-rank residual structure (p800_lowrank).
#
...
for (h2y in h2.foc) {
  for (h2s in h2.sec) {
    for (comm in comms) {
      ...
      # Export results:
      write.csv(results, sprintf("p800_lowrank/results/h2s%s/8%s_p800_lowrank_results_multiMLP_%s_h2y%s_comm%s_h2s%s_11to20.csv",
                                 h2s, lab, CV, h2y, comm, h2s))

      list.save(histories, sprintf("p800_lowrank/results/h2s%s/8%s_p800_lowrank_results_multiMLP_%s_h2y%s_comm%s_h2s%s_11to20.RData",
                                   h2s, lab, CV, h2y, comm, h2s))

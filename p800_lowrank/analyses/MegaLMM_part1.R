#!/usr/bin/env Rscript
# This script produces intermediate results 1 to 4 for the MegaLMM CV1 and CV2
...
#   intermediate results 1 to 4, for each combination of CV, h2y, h2s, and comm.
#
  ...
  for (h2y in h2.foc) {
    for (comm in comms) {
      for (h2s in h2.sec) {
        ...
        # Export results:
        write.csv(results, sprintf("p800_lowrank/results/h2s%s/12%s_p800_lowrank_results_MegaLMM_%s_h2y%s_comm%s_h2s%s_1to4.csv",

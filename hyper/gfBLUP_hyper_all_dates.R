...
loadings$Date <- substr(rownames(loadings), 7, 12)
loadings$Wavelength <- substr(rownames(loadings), 3, 5)
loadings <- tidyr::pivot_longer(loadings, 1:13, names_to = "Factor", values_to = "Loading")
loadings$Wavelength <- as.numeric(loadings$Wavelength)
loadings$Factor <- factor(loadings$Factor, levels = paste0("F", 1:13))
loadings$Date <- factor(loadings$Date, levels = unique(loadings$Date))
loadings$Important <- logical(nrow(loadings))

(important <- names(gencors[, "Y"])[which(abs(gencors[, "Y"]) >= 0.1)][-6])
loadings[which(loadings$Factor %in% important), "Important"] <- TRUE
loadings[which(!(loadings$Factor %in% important)), "Important"] <- FALSE
loadings$Important <- factor(loadings$Important, levels = c(TRUE, FALSE),
                             labels = c("Yes", "No"))
...
# Plotting:
ggplot(loadings, aes(x = Wavelength, y = Loading, color = Factor, alpha = Important)) +
  annotation_custom(g, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
  geom_line(linewidth = 1.5) +
  facet_wrap(vars(Date), nrow = 2) +
  scale_color_manual(values = unname(Polychrome::dark.colors(13))) +
  scale_alpha_manual(values = c("Yes" = 1, "No" = 0.15)) +
  ylim(c(-0.5, 1)) +
...
round(gencors[, "Y"], 2)[which(abs(gencors[, "Y"]) >= 0.1)][-6]

ggsave("plots/gfBLUP_hyper_all_dates.png", width = 24, height = 16, units = "cm")

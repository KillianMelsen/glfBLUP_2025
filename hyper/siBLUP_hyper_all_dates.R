...
# Run siBLUP:
RESULT <- gfBLUPold::siBLUP(d = d, K = K, CV = "CV2", do.parallel = FALSE, t.RF = 1, verbose = TRUE, sepExp = FALSE)

# Getting the SI-Y genetic correlation and SI weights (gamma):
gamma <- as.data.frame(RESULT$gamma)
gencors <- cov2cor(RESULT$Vg)
gamma$Date <- substr(rownames(gamma), 7, 12)
gamma$Wavelength <- substr(rownames(gamma), 3, 5)
gamma$Wavelength <- as.numeric(gamma$Wavelength)
gamma$Date <- factor(gamma$Date, levels = unique(gamma$Date))
...
# Plotting:
ann <- data.frame(Gamma = 0.05,
                  Date = "150225",
                  Wavelength = 700)

ggplot(gamma, aes(x = Wavelength, y = Gamma)) +
  ...
  geom_text(data = ann, label = paste(("rho[(SI*', '* Y)]^g * ' = ' *"), round(gencors["penSI", "Y"], 2)),
            color = "white", hjust = 0, size = 2.4, parse = TRUE) +
  ylab("SI Weight") + xlab("Wavelength (nm)")

ggsave("plots/siBLUP_hyper_all_dates.png", width = 24, height = 8, units = "cm")

...
# Creating run ID:
run_ID <- "hyper/megalmm_states/MegaLMM_hyper_all_dates_M25"
...
save(Lambda_samples, file = "hyper/megalmm_hyper_all_dates_arrays/LAMBDA_M25.RData")
save(pred_samples, file = "hyper/megalmm_hyper_all_dates_arrays/PREDS_M25.RData")
...
ggsave("plots/MegaLMM_hyper_all_dates_M25.png", width = 24, height = 7.5, units = "cm")

# M = 25 traceplotting =========================================================
rm(list = ls())
if (!("Lambda_samples" %in% ls() & "pred_samples" %in% ls())) {
  load("hyper/megalmm_hyper_all_dates_arrays/LAMBDA_M25.RData")
  load("hyper/megalmm_hyper_all_dates_arrays/PREDS_M25.RData")
}
...
ggsave("plots/MegaLMM_hyper_all_dates_traceplot_Y_M25.png", width = 24, height = 8, units = "cm")
...
for (f in paste0("F", 1:dim(Lambda_samples)[2])) {
  ...
  ggsave(filename = sprintf("plots/MegaLMM_hyper_all_dates_traceplot_WL_%s_M25.png", f), width = 24, height = 6.1, units = "cm")
    ...
    ggsave(filename = sprintf("plots/MegaLMM_hyper_all_dates_traceplot_WL_%s_M25.png", f), width = 24, height = 6.9, units = "cm")

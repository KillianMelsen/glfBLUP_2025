...
pseudoCRD.VEGsplines <- readRDS("hyper/data_generation/pseudoCRD_VEGsplines.rds")
...
for (run in 1:n.datasets) {
  ...
  datalist.nosplines <- list(data = as.data.frame(pseudoCRD.nosplines[, 3:624]),
                             pred.target = unique(as.data.frame(pseudoCRD.nosplines.test[, c(3, 625)])),
  ...
  names(datalist.nosplines$data)[622] <- "Y"
  rlist::list.save(datalist.nosplines, file = sprintf("hyper/datasets/nosplines/hyper_dataset_%d.RData", run))

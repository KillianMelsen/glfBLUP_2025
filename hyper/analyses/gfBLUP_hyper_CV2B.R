...
for (CV in c("CV2B")) {
  ...
  cl <- parallel::makeCluster(n.cores, outfile = "log_gfBLUP_hyper_CV2B.txt")
  ...
  invisible(
    par.results <- foreach::foreach(i = 1:length(work), .packages = c("rlist", "tictoc", "gfBLUPold", "gfBLUP"), .combine = "c") %dopar% {
      ...
      for (run in 1:length(par.work)) {
        ...
        # The training and test set data was scaled separately during the generation of the datasets, but in CV2B we want to use both test and
        # training data for the unsupervised part of gfBLUP. So we have to scale them together:
        d[sec] <- sapply(d[sec], scale)
        
        ### 2. Redundancy filter the secondary features using training AND TEST data ---------------------------------------------------------
        temp <- gfBLUP::redundancyFilter(data = d[c("G", sec)], tau = 0.95, verbose = TRUE)
        d.RF <- cbind(temp$data.RF, d[foc])
        d.train.RF <- d.train[names(d.RF)]
        sec.RF <- names(d.RF[2:(ncol(d.RF) - 1)])
        
        ### 3. Regularization ----------------------------------------------------------------------------------------------------------------
        folds <- gfBLUP::createFolds(genos = unique(as.character(d.RF$G)))
        tempG <- gfBLUP::regularizedCorrelation(data = d.RF[c("G", sec.RF)], folds = folds, what = "genetic", dopar = FALSE, verbose = TRUE)
        tempE <- gfBLUP::regularizedCorrelation(data = d.RF[c("G", sec.RF)], folds = folds, what = "residual", dopar = FALSE, verbose = TRUE)
        ...
        FM.fit <- gfBLUP::factorModel(data = d.RF[c("G", sec.RF)], cormat = Rg.RF.reg, what = "genetic", verbose = TRUE)
        ...
        selection <- gfBLUP::factorSelect(d.final, procedure = "leaps", verbose = TRUE)
        
        #### 7. Multi-trait genomic prediction -----------------------------------------------------------------------------------------------
        temp <- gfBLUP::gfBLUP(data = d.final, selection = selection, K = K, sepExp = FALSE, verbose = TRUE)

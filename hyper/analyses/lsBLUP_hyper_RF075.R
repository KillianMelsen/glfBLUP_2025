...
for (CV in c("CV1", "CV2")) {
  ...
  invisible(
    par.results <- foreach::foreach(i = 1:length(work), .packages = c("rlist", "tictoc", "gfBLUPold"), .combine = "c") %dopar% {
      ...
      for (run in 1:length(par.work)) {
        ...
        ### Model ##############################################################
        tic(run)
        RESULT <- gfBLUPold::lsBLUP(d = d, K = K, CV = CV, do.parallel = FALSE, sepExp = FALSE, t.RF = 0.75, verbose = FALSE)
        toc(log = TRUE)
        ########################################################################
  ...
  # Export results:
  write.csv(results, sprintf("analyses_results/hyper/11%s_hyper_results_lsblup_RF075_%s.csv", lab, CV))
  
  list.save(extra, sprintf("analyses_results/hyper/11%s_hyper_extra_results_lsblup_RF075_%s.RData", lab, CV))

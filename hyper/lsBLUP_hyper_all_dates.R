...
coefs$Date <- substr(rownames(coefs), 7, 12)
coefs$Wavelength <- substr(rownames(coefs), 3, 5)
coefs$Wavelength <- as.numeric(coefs$Wavelength)
coefs$Date <- factor(coefs$Date, levels = unique(coefs$Date))
...
# Plotting:
ann <- data.frame(Coefficient = 1,
                  Date = "150225",
                  Wavelength = 700)

ggplot(coefs, aes(x = Wavelength, y = Coefficient)) +
  ...
  geom_text(data = ann, label = paste(("rho[(LS*', '* Y)]^g * ' = ' *"), round(gencors["LSP", "Y"], 2)),
  ...
  #          color = "white", parse = TRUE, size = 6, hjust = 0) +
  ylab("LSP Coefficient") + xlab("Wavelength (nm)")

ggsave("plots/lsBLUP_hyper_all_dates.png", width = 24, height = 8, units = "cm")

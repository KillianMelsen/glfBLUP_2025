...
Note that a copy of this repository is also available at https://github.com/KillianMelsen/glfBLUP_2025. The github repository contains all intermediate results, but excludes the datafiles for different replications and raw hyperspectral data (raw hyperspectral data is available from J. Crossa upon reasonable request). The datafiles for the different replications can be generated by following the instructions in the `Reproducibility instructions` section. Note that the github repository also excludes the posterior arrays produced by MegaLMM for the single date analyses due to file size. These arrays can be produced using the [`masterscript_hyper.R`](masterscript_hyper.R) masterscript.
...
-   [`hyper_datafiles`](hyper_datafiles) this folder contains the raw data for the hyperspectral dataset. Note that the public github repository does not contain the raw data. The raw data for the hyperspectral B5IR and HEAT datasets is available from J. Crossa upon reasonable request. Also note that the datafiles to reproduce the intermediate hyperspectral B5IR and HEAT results can still be generated from the pre-processed data using the appropriate scripts, ensuring full reproducibility.
...
-   [`README.html`](README.html) this is the html version of the readme.
...
1. [`hyper_1415B5IR/data_generation/hyper_preprocessing.R`](hyper_1415B5IR/data_generation/hyper_preprocessing.R), skip this script if you are using the public github repository as no raw data is publicly available.
2. [`hyper_1415B5IR/data_generation/generate_hyper_datasets.R`](hyper_1415B5IR/data_generation/generate_hyper_datasets.R)
3. [`hyper_1415HEAT/data_generation/hyper_preprocessing.R`](hyper_1415HEAT/data_generation/hyper_preprocessing.R), skip this script if you are using the public github repository as no raw data is publicly available.
...
## Hyperspectral B5IR ([`hyper_1415B5IR`](hyper_1415B5IR))
See the paper at https://doi.org/10.1534/g3.118.200856 for details on this dataset.

### Data generation
There are two scripts to generate all required datafiles. The [`hyper_1415B5IR/data_generation/hyper_preprocessing.R`](hyper_1415B5IR/data_generation/hyper_preprocessing.R) script takes the raw hyperspectral B5IR data and pre-processes it. Note that the raw data is not available in the public github repository and can be obtained from J. Crossa upon reasonable request. However, the output of the pre-processing script is available, and can be used to generate all required datafiles. The [`hyper_1415B5IR/data_generation/generate_hyper_datasets.R`](hyper_1415B5IR/data_generation/generate_hyper_datasets.R) then generates the actual datafiles and samples training and test sets for the 250 replications.
...
### Data generation
There are two scripts to generate all required datafiles. The [`hyper_1415HEAT/data_generation/hyper_preprocessing.R`](hyper_1415HEAT/data_generation/hyper_preprocessing.R) script takes the raw hyperspectral HEAT data and pre-processes it.  Note that the raw data is not available in the public github repository and can be obtained from J. Crossa upon reasonable request. However, the output of the pre-processing script is available, and can be used to generate all required datafiles. The [`hyper_1415HEAT/data_generation/generate_hyper_datasets.R`](hyper_1415HEAT/data_generation/generate_hyper_datasets.R) then generates the actual datafiles and samples training and test sets for the 250 replications.
...
### Analyses
There are 15 scripts that perform the analyses on the simulated datasets with low-rank residual structure:

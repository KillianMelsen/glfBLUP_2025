...
# results for the simulated data with low-rank residual structure (p800_lowrank),
# as well as figure 6 of the main text.
#
...
# `p800_lowrank/datasets/`, or run the following scripts below:
...
# This script simulates all datasets using a low-rank residual structure.
source("p800_lowrank/data_generation/generate_sim_p800_lowrank_datasets.R"); reset()

## Step II: train/test division =================================================
# This script samples the training and test sets for all replications.
source("p800_lowrank/data_generation/traintest_sim_p800_lowrank_datasets.R"); reset()

## Step III: univariate analysis ===============================================
# This script runs all replications for the univariate model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/univariate.R"); reset()

## Step IV: benchmark analysis =================================================
# This script runs all replications for the benchmark model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/benchmark.R"); reset()

## Step V: glfBLUP analysis ====================================================
# This script runs all replications for the glfBLUP model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/glfBLUP.R"); reset()

## Step VI: MegaLMM analysis ===================================================
# The 5 scripts below run all replications for the MegaLMM model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/MegaLMM_part1.R"); reset()
source("p800_lowrank/analyses/MegaLMM_part2.R"); reset()
source("p800_lowrank/analyses/MegaLMM_part3.R"); reset()
source("p800_lowrank/analyses/MegaLMM_part4.R"); reset()
source("p800_lowrank/analyses/MegaLMM_part5.R"); reset()
# This script simply merges the intermediate results produced by the 5 scripts above.
source("p800_lowrank/misc/MegaLMM_merge.R"); reset()

## Step VII: lsBLUP analysis ===================================================
# This script runs all replications for the lsBLUP model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/lsBLUP.R"); reset()

## Step VIII: siBLUP analysis ==================================================
# The 2 scripts below run all replications for the siBLUP model on all the simulated
# p800_lowrank data.
source("p800_lowrank/analyses/siBLUP_part1.R"); reset()
source("p800_lowrank/analyses/siBLUP_part2.R"); reset()
# This script simply merges the intermediate results produced by the 2 scripts above.
source("p800_lowrank/misc/siBLUP_merge.R"); reset()

## Step IX: MultiMLP analysis ==================================================
# The 2 scripts below run all CV1 replications for the multiMLP model on all the
# simulated p800_lowrank data.
source("p800_lowrank/analyses/CV1_multiMLP_part1.R"); reset()
source("p800_lowrank/analyses/CV1_multiMLP_part2.R"); reset()
# The 2 scripts below run all CV2 replications for the multiMLP model on all the
# simulated p800_lowrank data.
source("p800_lowrank/analyses/CV2_multiMLP_part1.R"); reset()
source("p800_lowrank/analyses/CV2_multiMLP_part2.R"); reset()
# The 2 scripts below simply merge the intermediate results produced by the scripts
# above.
source("p800_lowrank/misc/CV1_multiMLP_merge.R"); reset()
source("p800_lowrank/misc/CV2_multiMLP_merge.R"); reset()

## Step X: Result Plotting =====================================================
# This script uses all p800_lowrank intermediate results to produce figure 6 of
# the main text.
source("p800_lowrank/plot_p800_lowrank_results.R"); reset()

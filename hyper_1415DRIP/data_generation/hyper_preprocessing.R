...
df <- df %>% dplyr::filter(year == "Y1415" & conditions == "Drip")
...
for (d in dates) {
  ...
  if (i == 1) {
    ...
    saveRDS(coords, "hyper_1415DRIP/data_generation/coords.rds")
  }
  i <- i + 1
}
toc()
saveRDS(corr.list, "hyper_1415DRIP/data_generation/corr_list.rds")



rm(list = setdiff(ls(), c("dates")))
corr.list <- readRDS("hyper_1415DRIP/data_generation/corr_list.rds")
...
data.wide <- as.data.frame(tidyr::pivot_wider(data,
                                              ...
                                              values_from = 11:72))
saveRDS(data.wide, "hyper_1415DRIP/data_generation/hyper_pseudoCRD_nosplines.rds")
...
data.splines.wide <- as.data.frame(tidyr::pivot_wider(data.splines,
                                                      ...
                                                      names_glue = "{.value}_{date}",
                                                      values_from = 11:72))
saveRDS(data.splines.wide, "hyper_1415DRIP/data_generation/hyper_pseudoCRD_splines.rds")
...
# Loading data:
yield_1415 <- readxl::read_xlsx("hyper_1415DRIP/data_generation/EYT_all_data_Y13_14_to_Y15_16.xlsx", sheet = 2)
genotypes <- vroom::vroom("hyper_1415DRIP/data_generation/Krause_et_al_2018_Genotypes.csv")
pseudoCRD.nosplines <- readRDS("hyper_1415DRIP/data_generation/hyper_pseudoCRD_nosplines.rds")
pseudoCRD.splines <- readRDS("hyper_1415DRIP/data_generation/hyper_pseudoCRD_splines.rds")
pseudoCRD.VEGsplines <- readRDS("hyper_1415DRIP/data_generation/hyper_pseudoCRD_VEGsplines.rds")
coords <- readRDS("hyper_1415DRIP/data_generation/coords.rds")
...
yield_1415_B5IR <- dplyr::filter(yield_1415, env == "DRIP")
...
gp_ready.VEGsplines <- dplyr::filter(gp_ready.VEGsplines, gid %in% rownames(K))
...
saveRDS(gp_ready.nosplines, "hyper_1415DRIP/data_generation/pseudoCRD_nosplines.rds")
saveRDS(gp_ready.splines, "hyper_1415DRIP/data_generation/pseudoCRD_splines.rds")

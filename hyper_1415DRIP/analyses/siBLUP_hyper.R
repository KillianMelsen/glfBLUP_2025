...
# Runtime: ~ 25mins for 250 datasets using 18 workers.
prep <- "splines"
# Loading libraries:
library(rlist)
library(tictoc)
...
for (CV in c("CV1", "CV2")) {
  ...
  # Export results:
  write.csv(results, sprintf("hyper_1415DRIP/results/%s/5%s_hyper_results_siblup_%s.csv", prep, lab, CV))
  
  list.save(extra, sprintf("hyper_1415DRIP/results/%s/5%s_hyper_extra_results_siblup_%s.RData", prep, lab, CV))

...
for (CV in c("CV1", "CV2")) {
  ...
  # Export results:
  write.csv(results, sprintf("hyper_1415DRIP/results/%s/11%s_hyper_results_lsblup_%s.csv", prep, lab, CV))
  
  list.save(extra, sprintf("hyper_1415DRIP/results/%s/11%s_hyper_extra_results_lsblup_%s.RData", prep, lab, CV))

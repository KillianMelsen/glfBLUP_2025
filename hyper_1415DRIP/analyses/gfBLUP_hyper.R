...
invisible(
  par.results <- foreach::foreach(i = 1:length(work), .packages = c("rlist", "tictoc", "gfBLUPold", "gfBLUP"), .combine = "c") %dopar% {
    ...
    for (run in 1:length(par.work)) {
      ...
      datalist <- list.load(file = sprintf("hyper_1415DRIP/datasets/%s/hyper_dataset_%d.RData", prep, par.work[run]))
...
# Export results:
write.csv(CV1.results, sprintf("hyper_1415DRIP/results/%s/3a_hyper_results_gfblup_CV1.csv", prep))
write.csv(CV2.results, sprintf("hyper_1415DRIP/results/%s/3b_hyper_results_gfblup_CV2.csv", prep))

list.save(extra, sprintf("hyper_1415DRIP/results/%s/3_hyper_extra_results_gfblup.RData", prep))

...
par.results <- foreach::foreach(i = 1:length(work), .packages = c("rlist", "tictoc"), .combine = "c") %dopar% {
  ...
  for (run in 1:length(par.work)) {
    ...
    datalist <- list.load(file = sprintf("hyper_1415DRIP/datasets/splines/hyper_dataset_%d.RData", par.work[run]))
...
# Export results:
write.csv(results, "hyper_1415DRIP/results/2_hyper_results_univariate.csv")

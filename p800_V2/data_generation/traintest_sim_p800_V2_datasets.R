...
for (h2s in h2.sec) {
  for (comm in comms) {
    ...
    cl <- parallel::makeCluster(5, outfile = sprintf("logs/traintest_p800_V2_h2s%s_comm%s.txt", h2s, comm))
    ...
    foreach::foreach(i = 1:length(h2.foc)) %dopar% {
      ...
      for (sim in 1:n.sim) {
        ...
        datalist <- list.load(file = sprintf("p800_V2/datasets/p800_V2_h2y%s_comm%s_h2s%s_dataset_%d.RData",
                                             h2y, comm, h2s, sim))
        ...
        # Overwriting dataset:
        list.save(datalist, file = sprintf("p800_V2/datasets/p800_V2_h2y%s_comm%s_h2s%s_dataset_%d.RData",
                                           h2y, comm, h2s, sim))

  ...
  for (h2s in h2.sec) {
    for (comm in comms) {
      ...
      for (sim in first:last) {
        ...
        datalist <- list.load(sprintf("p800_V2/datasets/p800_V2_h2y%s_comm%s_h2s%s_dataset_%d.RData", h2y, comm, h2s, sim))
      ...
      # Export results:
      write.csv(results, sprintf("p800_V2/results/h2s%s/8%s_p800_V2_results_multiMLP_%s_h2y%s_comm%s_h2s%s_11to20.csv",
                                 h2s, lab, CV, h2y, comm, h2s))

      list.save(histories, sprintf("p800_V2/results/h2s%s/8%s_p800_V2_results_multiMLP_%s_h2y%s_comm%s_h2s%s_11to20.RData",
                                   h2s, lab, CV, h2y, comm, h2s))

  ...
  for (h2y in h2.foc) {
    ...
    foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
      ...
      for (sim in first:last) {
        ...
        cat(sprintf("Running siBLUP on %s p800_V2 dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
      ...
      # Export results:
      write.csv(results, sprintf("p800_V2/results/h2s%s/5%s_p800_V2_results_siblup_%s_h2y%s_comm%s_h2s%s_51to100.csv",
                                 h2s, lab, CV, h2y, comm, h2s))
      
      list.save(extra, file = sprintf("p800_V2/results/h2s%s/5%s_p800_V2_extra_results_siblup_%s_h2y%s_comm%s_h2s%s_51to100.RData",
                                      h2s, lab, CV, h2y, comm, h2s))

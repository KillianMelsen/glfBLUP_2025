...
for (CV in CVs) {
  for (h2y in h2.foc) {
    for (comm in comms) {
      for (h2s in h2.sec) {
        tic(sprintf("MegaLMM p800_V2 combi %d / %d, (CV = %s, h2y = %s, comm = %s, h2s = %s)", # tic 2
        ...
        for (sim in first:last) {
          ...
          cat(sprintf("Running MegaLMM on %s p800_V2 dataset %d / %d (part %s, dataset %s), (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
          ...
          # Creating run ID:
          run_ID <- sprintf("p800_V2/megalmm_states/%s_p800_V2_dataset_h2y%s_comm%s_h2s%s_%d", CV, h2y, comm, h2s, sim)
        ...
        # Export results:
        write.csv(results, sprintf("p800_V2/results/h2s%s/12%s_p800_V2_results_MegaLMM_%s_h2y%s_comm%s_h2s%s_17to20.csv",

...
for (CV in CVs) {
  for (h2s in h2.sec) {
    ...
    tic(sprintf("lsBLUP p800_V2 combi %d / %d, (CV = %s, h2s = %s)", combi, combis, CV, h2s))
    
    cl <- parallel::makeCluster(15, outfile = sprintf("logs/lsBLUP_sim_p800_V2_h2s%s_%s.txt", h2s, CV))
    ...
    foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
      ...
      for (sim in 1:n.sim) {
        ...
        cat(sprintf("Running lsBLUP on %s p800_V2 dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
      ...
      # Export results:
      write.csv(results, sprintf("p800_V2/results/h2s%s/11%s_p800_V2_results_lsblup_%s_h2y%s_comm%s_h2s%s.csv",
                                 h2s, lab, CV, h2y, comm, h2s))
      list.save(extra, file = sprintf("p800_V2/results/h2s%s/11%s_p800_V2_extra_results_lsblup_%s_h2y%s_comm%s_h2s%s.RData",
                                      h2s, lab, CV, h2y, comm, h2s))

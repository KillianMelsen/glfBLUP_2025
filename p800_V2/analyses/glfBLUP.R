...
for (h2y in h2.foc) {
  ...
  tic(sprintf("glfBLUP CV1/CV2 p800_V2 combi %d / %d, (h2y = %s)", combi, combis, h2y))
  
  cl <- parallel::makeCluster(9, outfile = sprintf("logs/glfBLUP_sim_p800_V2_h2y%s.txt", h2y))
  ...
  foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
    ...
    for (sim in first:last) {
      ...
      cat(sprintf("Running glfBLUP CV1/CV2 on p800_V2 dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
    ...
    list.save(extra, file = sprintf("p800_V2/results/h2s%s/3a_p800_V2_extra_results_glfblup_CV1_h2y%s_comm%s_h2s%s.RData", h2s, h2y, comm, h2s))
    list.save(extra, file = sprintf("p800_V2/results/h2s%s/3b_p800_V2_extra_results_glfblup_CV2_h2y%s_comm%s_h2s%s.RData", h2s, h2y, comm, h2s))

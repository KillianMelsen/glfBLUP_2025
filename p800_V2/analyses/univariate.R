...
for (h2s in h2.sec) {
  ...
  cl <- parallel::makeCluster(15, outfile = sprintf("logs/univariate_sim_p800_V2_h2s%s.txt", h2s))
  ...
  foreach::foreach(i = 1:nrow(par.combis), .packages = c("rlist", "tictoc")) %dopar% {
    ...
    for (sim in 1:n.sim) {
      ...
      cat(sprintf("Running univariate on p800_V2 dataset %d / %d, (h2s = %s, comm = %s, h2y = %s), combi %d / %d\n",
    ...
    # Export results:
    write.csv(results, sprintf("p800_V2/results/h2s%s/2_p800_V2_results_univariate_h2y%s_comm%s_h2s%s.csv",
                               h2s, h2y, comm, h2s))

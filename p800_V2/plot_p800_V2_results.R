      ...
      for (comm in comms) {
        if (model != "univariate") {
          for (CV in CVs) {
            ...
            accs <- read.csv(sprintf("p800_V2/results/h2s%s/%s%s_p800_V2_results_%s_%s_h2y%s_comm%s_h2s%s.csv",
          ...
          accs <- read.csv(sprintf("p800_V2/results/h2s%s/2_p800_V2_results_univariate_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))$acc
          stopifnot(length(accs) == 20 | length(accs) == 100)
          stopifnot(length(unique(accs)) == 20 | length(unique(accs)) == 100)
          acc <- median(read.csv(sprintf("p800_V2/results/h2s%s/2_p800_V2_results_univariate_h2y%s_comm%s_h2s%s.csv", h2s, h2y, comm, h2s))$acc)
...
ggsave(filename = "plots/p800_V2.png", dpi = 640, width = 25, height = 27.5, units = "cm")
...
ggsave(filename = "plots/p800_V2_main.png", dpi = 640, width = 25, height = 27.5, units = "cm")
